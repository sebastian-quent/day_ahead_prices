...
- [x] Dependency manifest — `pyproject.toml`/`poetry.lock` added, Poetry-managed like Production. Shared deps pinned to match Production's `pyproject.toml`/`poetry.lock` exactly (`prefect` exact `3.4.14`, others same `~=` tilde ranges: `pandas`, `sqlalchemy`, `psycopg2`, `requests`, `paramiko`, `xmltodict`, `zeep`, `pytz`, `lxml`, `openpyxl`, `python-dateutil`). One deliberate deviation: `quent_core` pinned to git rev `v1.0.159-streaming-functions`, not Production's `v1.0.158` — the `streaming` module this project depends on (`core/price_store.py`'s NATS publish) doesn't exist before that ref (its internal `__version__` reports `1.0.160-alpha.4` despite the `v1.0.159` ref name — a pre-release/tag-naming mismatch in the upstream `quent_core` repo itself, confirmed harmless: the module content is what matters, not the label. Corrected here 2026-07-21 — this doc previously mis-stated the pin as literally `v1.0.160-alpha.4`, which pyproject.toml has never actually said). Kept independent (own `pyproject.toml`, `package-mode = false`, own `.venv`) rather than merged into Production's, so a future merge is a straightforward pin reconciliation, not a rewrite. Two additional explicit pins added 2026-07-20 to fix locally-broken transitive deps of `prefect==3.4.14` (see Known gaps): `importlib-metadata = ">=4.4"` and `fastapi = "~=0.115.6"`. `streamlit ~=1.43.0` added 2026-07-21 for `monitoring/coverage.py` (see Monitoring section).
...
**`monitoring/coverage.py`** — Streamlit prototype (run with `poetry run streamlit run monitoring/coverage.py`), for eyeballing "which zone from which scraper is in already" without querying the DB by hand. Separate from the completeness check above, not a replacement for it: this is interactive/manual, per-**source** granularity (not just zone-level), and has no deployment/alerting of its own.
- Date picker for one delivery day at a time (default tomorrow), queries `PriceStore.get(product="DAY_AHEAD", ...)` for that day's UTC bounds, then groups results **by source** — each scraper gets its own block listing the zones it landed as chips, e.g. `OMIE` → `PT (96/96)`, `ES (96/96)`.
...
- Grouped by `(source, bidding_zone, market)` before summing to actual/expected per zone, not straight to `(source, bidding_zone)` — GB lands as two separate `market` rows per source (N2EX hourly + GbHalfHour half-hourly) with different resolutions each; collapsing too early picked one arbitrary resolution and miscomputed `expected` (caught live: Nord Pool GB showed `72/48` instead of `72/72`).
- `IN_SCOPE_ZONES` duplicated from `monitoring/day_ahead_completeness.py` rather than shared via `core/` — same "revisit once a third consumer needs it" reasoning as that module already uses, now with a second consumer.
...
- Status colors/chip styling follow the team's data-viz status palette (fixed hex, not theme-dependent); no legend — green/amber/red read as fine/partial/missing without one.
- **Known limitation, by design for now**: reads `prod.prices` only, so it shows *whether* data is in, not *why* it's missing (a source erroring vs. simply not having published yet look identical). Reasonable per-source publish lag (e.g. SEMO/IE's documented day-later batch publish, see SEMO to-do) will show as red without it being a real gap. A logs-based system with actual run/error status is a separate, later piece of work (see Monitoring intro above) — this dashboard would plug into that as an additional data source without changing its zone × source grid shape.
...
**Global interpreter had a stale, unpinned `quent_core` install, causing a hard-to-diagnose bug.** This project's own `.venv` is correctly Poetry-managed (in-project, matches the git rev pinned in `pyproject.toml`) — confirmed via `poetry env info`. But the *global* interpreter (`AppData\Local\Programs\Python\Python312`, used before this project adopted Poetry) had its own `quent_core` installed via plain `pip`, unpinned and out of sync with the pinned rev. Anything run with a bare `python` instead of `poetry run` silently picked up that stale version instead of the pinned one — root cause of a `publish_many` tuple-unpacking bug in the NATS streaming path. Stray global `quent_core` uninstalled (2026-07-20, confirmed via user) — Production is separately Poetry-managed too, so nothing legitimately depended on it being there. Discipline (always `poetry run ...`), no runtime guard.
- **Update 2026-07-21**: the project's own `.venv` had drifted too, not just the global interpreter — `poetry install` (run when adding `streamlit` for `dashboard/coverage.py`) found the venv actually had a newer, untracked `quent_core` commit installed (`f705d5c`) than the one pinned in `poetry.lock` (`d42834d`, the `v1.0.159-streaming-functions` ref). `poetry install` silently corrected it back to the pinned commit; confirmed harmless since `d42834d` still contains `quent_core.streaming` (internal `__version__` reports `1.0.160-alpha.4`, see Dependency manifest to-do). No action needed beyond noting it — worth an occasional `poetry install`/`poetry check` if this class of drift recurs.
